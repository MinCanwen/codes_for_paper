...
# 构建输出文件夹路径
output_dir = os.path.join(current_dir, f"001_G_{pid}")
print(output_dir)
# 如果目录不存在，则创建
...
class PINNInference():
    def __init__(self, 
        ...
        # settings lambda初值
        self.lambda_1 = torch.ones(16, 1, requires_grad=True).to(device)
...
lambda_1_history = []
layers = [2,25,20,20,20,20,5]
model = PINNInference(x_max=1,t_max=1,measurement=m_sample,f=f_sample ,Nf=Nf, layers=layers, device=device, a=1, b=1, c=1, d=1)
model.train_inverse(niter=5001,Ir=0.005,lambda1=1,lambda2=0.1,lambda3=1e-9,plot_num=1000,dt=1.0,dx=1.0)
print_pde(model.lambda_1, rhs_des)
model.train_inverse(niter=5001,Ir=0.003,lambda1=1,lambda2=1,lambda3=1e-8,plot_num=1000,dt=0.5,dx=0.5)
print_pde(model.lambda_1, rhs_des)
figure_compare(name='pertraining1')
model.train_inverse(niter=10001,Ir=0.001,lambda1=1,lambda2=1,lambda3=1e-7,plot_num=1000,dt=0.3,dx=0.3)
...
for i in range(7):  # 第3次训练循环
    ...
    # 训练模型2
    model.train_inverse(niter=10001, Ir= 0.001, lambda1=1, lambda2=10, lambda3=1e-5, lambda4=10, plot_num=1000, freezing=True,dt=0.3,dx=0.3)
    
    # 打印结果2
    print_pde(model.lambda_1, rhs_des)
 
model.train_inverse(niter=10001, Ir= 0.0005, lambda1=1, lambda2=100, lambda3=0, lambda4=10, plot_num=1000, freezing=True,dt=0.1,dx=0.1)
# 转换为 NumPy 数组，便于后续操作
