...
model.train(niter=30001,Ir=0.0005,lambda1=1,lambda2=10,lambda3=1,plot_num=1000,freezing=True)
model.train(niter=30001,Ir=0.0005,lambda1=1,lambda2=100,lambda3=10,plot_num=1000,freezing=True)
figure_compare(name='train3')
model.train(niter=30001,Ir=0.0001,lambda1=1,lambda2=1000,lambda3=10,plot_num=1000,freezing=True)
figure_compare(name='train4')
model.train(niter=30001,Ir=0.0001,lambda1=1,lambda2=10000,lambda3=10,plot_num=1000,freezing=True)
figure_compare(name='train5')
model.train(niter=30001,Ir=0.00005,lambda1=1,lambda2=50000,lambda3=10,plot_num=1000,freezing=True)
figure_compare(name='train6')
for j in range(0):
    # 使用路径加载 .mat 文件
    data = scipy.io.loadmat(mat_file_path)
    t = 5*np.real(data['t'].flatten()[:,None])/10 #100 *  -85
    x = np.real(data['x'].flatten()[:,None])/10 # -25
    length2 = length2+5
    t = t[a0:a0+length2]
    t_domain = [t[0],t[-1]]
    ...
    model.train(niter=10001,Ir=0.00005,lambda1=1,lambda2=100,plot_num=1000,freezing=True)
    model.train(niter=10001,Ir=0.00001,lambda1=1,lambda2=1000,plot_num=1000,freezing=True)
    name = f"train_{j+5}"
    figure_compare(name)
