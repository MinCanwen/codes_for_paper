...
# 构建输出文件夹路径
output_dir = os.path.join(current_dir, f"50_loss_{pid}")
print(output_dir)
# 如果目录不存在，则创建
...
#model.train_inverse(niter=10001,Ir=0.001,lambda1=1,lambda2=0.1,lambda3=0,lambda4=0.1,plot_num=1000)
...
#model.train_inverse(niter=30001,Ir=0.001,lambda1=1,lambda2=10,lambda3=1e-9,lambda4=10,plot_num=1000)
#figure_compare(name='pertraining2')

for i in range(10):  # 第一次训练循环
    # 训练模型
    model.train_inverse(niter=5001, Ir= 0.001, lambda1=1, lambda2=1, lambda3=1e-4, lambda4=1,plot_num=1000)
    figure_compare(name=f'train_{i}')
    # 打印结果
    print_pde(model.lambda_1, rhs_des)

for i in range(0):  # 第二次训练循环
    # 剪枝和冻结权重
    model.dnn.prune_and_freeze_weights()
    
    # 训练模型2
    model.train_inverse(niter=3001, Ir= 0.001, lambda1=1, lambda2=10, lambda3=1e-3, lambda4=10.0, plot_num=1000,freezing=True)
    
    # 打印结果2
    print_pde(model.lambda_1, rhs_des)

#model.train_inverse(niter=50001, Ir= 0.001, lambda1=1, lambda2=100, lambda3=1e-4, lambda4=10.0, plot_num=1000,freezing=True)
for i in range(0):  # 第3次训练循环
    # 剪枝和冻结权重
...
#model.train_inverse(niter=10001, Ir= 0.001, lambda1=1, lambda2=100, lambda3=0, lambda4=10.0, plot_num=1000,freezing=True)
print_pde(model.lambda_1, rhs_des)
# 转换为 NumPy 数组，便于后续操作
