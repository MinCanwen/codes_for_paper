...
# 构建输出文件夹路径
output_dir = os.path.join(current_dir, f"0_50_{pid}")
...
model.train(niter=30001,Ir=0.00005,lambda1=1,lambda2=5000,lambda3=10,plot_num=1000,freezing=True)
figure_compare(name='train5')
for j in range(10):
    # 使用路径加载 .mat 文件
    data = scipy.io.loadmat(mat_file_path)
    ...
    model.train(niter=20001,Ir=0.00001,lambda1=1,lambda2=1000,plot_num=1000,freezing=True)
    name = f"train_{j+5}"
    figure_compare(name)
